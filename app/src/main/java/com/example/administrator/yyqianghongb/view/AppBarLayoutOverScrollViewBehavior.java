...
public class AppBarLayoutOverScrollViewBehavior extends AppBarLayout.Behavior {
    ...
    public boolean onLayoutChild(@NotNull CoordinatorLayout parent, @NotNull AppBarLayout abl, int layoutDirection) {
        ...
        if (scrollLayout == null) {
            scrollLayout = parent.findViewWithTag(TAG2);
            if (scrollLayout != null) {
                initial(abl);
            }
        }
        if (bottomLayout == null) {
            bottomLayout = parent.findViewWithTag(TAG3);
            if (bottomLayout != null) {
                initial(abl);
            }
        }
    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, @NonNull AppBarLayout child, View target, int dx, int dy, int[] consumed, int type) {
        if (mTargetView != null && scrollLayout!=null && ((dy < 0 && child.getBottom() >= mParentHeight) || (dy > 0 && child.getBottom() > mParentHeight))) {
...
//        ViewCompat.setTranslationY(bottomLayout,(int) (mTargetViewHeight / 2 * (mLastScale - 1)));
        mLastBottom = mParentHeight + (int) (mTargetViewHeight / 2 * (mLastScale - 1));
        abl.setBottom(mLastBottom);
        scrollLayout.setBottom(mLastBottom);
        target.setScrollY(0);
    }
...
//                        ViewCompat.setTranslationY(bottomLayout,-(mLastBottom- mParentHeight) * animation.getAnimatedFraction());
                        abl.setBottom((int) (mLastBottom - (mLastBottom - mParentHeight) * animation.getAnimatedFraction()));
                        scrollLayout.setBottom((int) (mLastBottom - (mLastBottom - mParentHeight) * animation.getAnimatedFraction()));
                    }
                });
                anim.start();
            } else {
...
//                ViewCompat.setTranslationY(bottomLayout,0f);
                abl.setBottom(mParentHeight);
                scrollLayout.setBottom(mParentHeight);
            }
        }
    }
}