...
import com.example.administrator.yyqianghongb.utils.ScreenUtil;
...
public class LoadViewController implements ILoadViewController {
    //默认circleimage大小
    static final int CIRCLE_DIAMETER = 50;
    ...
    @Override
    public View create(int loadColor) {
        linearLayout=new LinearLayout(mContext);
        linearLayout.setGravity(Gravity.CENTER);
        linearLayout.setBackgroundColor(0x00000000);
        linearLayout.setOrientation(LinearLayout.HORIZONTAL);
        View view = LayoutInflater.from(mContext).inflate(R.layout.foot_swiperefresh,null);
        SpinKitView spinKitView = view.findViewById(R.id.spinKitView);
        TextView tv = view.findViewById(R.id.tv);
        spinKitView.setColor(loadColor);
        tv.setTextColor(loadColor);
        ViewGroup.MarginLayoutParams marginLayoutParams = new ViewGroup.MarginLayoutParams(ScreenUtil.getScreenWidth(mContext), mCircleDiameter);
    ...
    public int move(int scrollDistance) {
        int scroll = (int) (scrollDistance * 0.5f);
    ...
    public void animateHideLoadMore(Animation.AnimationListener listener) {
        mParent.clearAnimation();
        mParent.scrollBy(0, -getCurrentHeight());
        reset();
    }
