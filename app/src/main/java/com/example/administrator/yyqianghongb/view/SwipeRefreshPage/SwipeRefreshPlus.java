...
public class SwipeRefreshPlus extends ViewGroup implements NestedScrollingParent,
    ...
    int circleViewIndex = ZIndex.TOP;
    ...
    private int loadColor = 0xff464646;
    ...
    public SwipeRefreshPlus(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        if (attrs != null) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.SwipeRefreshPlus);
            loadColor=typedArray.getColor(R.styleable.SwipeRefreshPlus_loadView_color, Color.parseColor("#464646"));
        }
     ...
     * 模式设置
     *
     * @param mode 模式 1:有上拉下拉功能 ，2：只有下拉 ，3：只有上拉 ， 4：啥都没有,
     */
     ...
     */
    private void setRefreshColorResources(@ColorRes int... colorResIds) {
     ...
     * @param colors 一组颜色
     */
    private void setRefreshColors(@ColorInt int... colors) {
        ensureTarget();
    ...
    private void setLoadMoreColors(@ColorInt int... colors) {
        ensureTarget();
    ...
    public void setLoadMore(boolean show) {
        ...
        MyLoge.addLoge("setLoadMore", oldStatus + "   " + show);
...
//            });
    ...
    public void setLoadMore2(boolean show) {
        final int height = mLoadViewController.getCurrentHeight();
        if (!show) {
            mLoadViewController.stopAnimation();
            MyLoge.addLoge("setLoadMore", "   queueIdle");
    ...
    public void stopLoading() {
        MyLoge.addLoge("stopLoading", "stopLoading");
    ...
    private void setNoMoreView() {
        LayoutParams noMoreLayoutParams = new LayoutParams(LayoutParams.MATCH_PARENT,
                LayoutParams.WRAP_CONTENT);
        mNoMoreView = LayoutInflater.from(getContext()).inflate(R.layout.foot_swiperefresh_nomore, null, false);
        TextView more_tv = mNoMoreView.findViewById(R.id.more_tv);
        more_tv.setTextColor(loadColor);
        mNoMoreView.setLayoutParams(noMoreLayoutParams);
    ...
    public void showNoMore(boolean show) {
        ...
        if (show && mNoMoreView != null && mLoadMoreView != mNoMoreView) {
     ...
     * 设置是否显示  已经到底了  ~
     *
     * @param isDown 是不是上拉,非down为上拉
     * @param list   接口返回的数据集合
     */
    boolean showNM;

    public <T> void setNoMore(String isDown, List<T> list) {
        if (mNoMoreView == null)
            setNoMoreView();

        if (!isDown.equals("down") && (list == null || list.size() == 0)) {
            showNM = true;
        } else {
            showNM = false;
        }
        mLoadViewController.showNoMore(showNM);
        if (showNM && mNoMoreView != null && mLoadMoreView != mNoMoreView) {
            mLoadMoreView.clearAnimation();
            detachViewFromParent(mLoadMoreView);
            removeDetachedView(mLoadMoreView, false);
            mLoadMoreView = mNoMoreView;
            addView(mNoMoreView, 0, mNoMoreView.getLayoutParams());
        } else if (!showNM && mLoadMoreView != mLoadViewController.getDefaultView()) {
            detachViewFromParent(mLoadMoreView);
            removeDetachedView(mLoadMoreView, false);
            mLoadMoreView = mLoadViewController.getDefaultView();
            addView(mLoadMoreView, 0);
    ...
    private void setLoadViewController(ILoadViewController controller) {
        this.mLoadViewController = controller;
        detachViewFromParent(mLoadMoreView);
        mLoadMoreView = mLoadViewController.create(loadColor);
        // requestLayout();
     ...
     * @param target
     */
    private void setScrollView(View target) {
    ...
    private void createProgressView() {
        ...
        this.mLoadMoreView = mLoadViewController.create(loadColor);
